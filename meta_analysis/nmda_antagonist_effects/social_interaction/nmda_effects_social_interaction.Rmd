---
title: "Effects of NMDA Receptor antagonists on social interaction"
...
# ---- User inputs ----
data_path  <- "meta_social_int_nmda_sb.xlsx"
sheet_name <- 1
...
pastel_fill <- function(n) {
  scales::alpha(
    colorRampPalette(c("#669989", "#DCE9E5"))(n),
    0.9
  )
}
...
This systematic review and meta-analysis evaluated the effects of NMDA receptor antagonists on social interaction outcomes in animal models. Effect sizes were calculated as Hedges’ g and synthesized using multilevel random-effects models to account for dependency between multiple outcomes within experiments and studies.
...
pC <- bubble_grid(
  dat_es,
  "species",
  "developmental_stage_induction",
  "Evidence map: species × developmental_stage_induction"
)
print(pC)
save_both(pC, "bubble_species_devstage", w = 9, h = 6)
```
...
pD <- bubble_grid(
  ...
  "species",
  "nmda_administration_route",
  "Evidence map: species × nmda_administration_route"
)
print(pD)
save_both(pD, "bubble_species_route", w = 9, h = 6)
```
```{r evidence_map_sex, message=FALSE, warning=FALSE}
pSex <- bubble_grid(
  dat_es,
  "species",
  "sex",
  "Evidence map: species × sex"
)
print(pSex)
save_both(pSex, "bubble_species_sex", w = 9, h = 6)
```
...
**Overall effect of NMDA receptor antagonists on social interaction.**\
...
```{r moderator_nmda_antagonist, message=FALSE, warning=FALSE}
mods_formula <- ~ 0 + nmda_antagonist
m_ant <- fit_mv(dat_es, V_main, mods = mods_formula)

print(m_ant$rob)

tbl_ant <- coef_table(m_ant$rob, "Moderator: NMDA antagonist")
write.csv(
  tbl_ant,
  "tables/mod_nmda_antagonist_coefficients.csv",
  row.names = FALSE
)

p_ant <- orchard_mod_plot(
  m_ant$rob,
  mod = "nmda_antagonist",
...
```
```{r moderator_route, message=FALSE, warning=FALSE}
mods_formula <- ~ 0 + nmda_administration_route
m_rt <- fit_mv(dat_es, V_main, mods = mods_formula)

print(m_rt$rob)

tbl_rt <- coef_table(
  m_rt$rob,
  "Moderator: Administration route"
)
write.csv(
  tbl_rt,
  "tables/mod_nmda_administration_route_coefficients.csv",
  row.names = FALSE
)

p_rt <- orchard_mod_plot(
  m_rt$rob,
  mod = "nmda_administration_route",
  title = "By Administration route"
)
...
save_both(
  p_rt,
  "orchard_by_nmda_administration_route",
  w = 9,
  h = 6
)
```
```{r moderator_schedule, message=FALSE, warning=FALSE}
mods_formula <- ~ 0 + nmda_schedule
m_sch <- fit_mv(dat_es, V_main, mods = mods_formula)
...
write.csv(
  tbl_sch,
  "tables/mod_nmda_schedule_coefficients.csv",
  row.names = FALSE
)

p_sch <- orchard_mod_plot(
  m_sch$rob,
  mod = "nmda_schedule",
  title = "By Schedule"
)
...
dat_cum <- dat_es %>%
  ...
  mutate(
    nmda_cumulative_exposure = na_if(nmda_cumulative_exposure, "NA"),
    nmda_cumulative_exposure = na_if(nmda_cumulative_exposure, ""),
    nmda_cumulative_exposure = as.numeric(nmda_cumulative_exposure)
  ) %>%
  filter(!is.na(nmda_cumulative_exposure))

V_cum <- make_V(dat_cum, rho = rho_main)
m_cum <- fit_mv(dat_cum, V_cum, mods = ~ nmda_cumulative_exposure)

print(m_cum$rob)
write.csv(coef_table(m_cum$rob, "Meta-reg: nmda_cumulative_exposure"), "tables/metareg_cumulative_exposure.csv", row.names = FALSE)

# Visual
pred_x <- seq(min(dat_cum$nmda_cumulative_exposure, na.rm = TRUE),
              max(dat_cum$nmda_cumulative_exposure, na.rm = TRUE), length.out = 80)
pred <- predict(m_cum$raw, newmods = pred_x)

p_cum <- ggplot(dat_cum, aes(x = nmda_cumulative_exposure, y = yi)) +
  geom_point(alpha = 0.25) +
  geom_line(data = data.frame(x = pred_x, y = pred$pred),
            aes(x = x, y = y), linewidth = 1, color = "black") +
  labs(title = "Meta-regression: cumulative exposure vs effect size (Rat+Mouse)",
       x = "nmda_cumulative_exposure", y = "Hedges g") +
  theme_minimal(base_size = 12)
p_cum
save_both(p_cum, "metareg_cumulative_exposure", w = 9, h = 6)
```
...
dat_cum <- dat_cum %>%
  filter(nmda_cumulative_exposure > 0) %>%
  mutate(
    log_nmda_cumulative_exposure = log10(nmda_cumulative_exposure)
  )
...
m_cum_log <- fit_mv(
  dat_cum,
  V_cum_log,
  mods = ~ log_nmda_cumulative_exposure
)

print(m_cum_log$rob)

write.csv(
  coef_table(m_cum_log$rob, "Meta-reg: log10(nmda_cumulative_exposure)"),
  "tables/metareg_log_cumulative_exposure.csv",
  row.names = FALSE
)

# Visual
pred_x_log <- seq(
  min(dat_cum$log_nmda_cumulative_exposure, na.rm = TRUE),
  max(dat_cum$log_nmda_cumulative_exposure, na.rm = TRUE),
  length.out = 80
)
...
p_cum_log <- ggplot(
  dat_cum,
  aes(x = log_nmda_cumulative_exposure, y = yi)
) +
  geom_point(alpha = 0.25) +
  geom_line(
  ...
  labs(
    title = "Meta-regression: log cumulative exposure vs effect size (Rat+Mouse)",
    x = expression(log[10]~"(NMDA cumulative exposure)"),
    y = "Hedges g"
  ) +
  theme_minimal(base_size = 12)

p_cum_log
save_both(p_cum_log, "metareg_log_cumulative_exposure", w = 9, h = 6)
```
...
write.csv(
  forest_tbl,
  "tables/effect_sizes_by_nmda_antagonist.csv",
  row.names = FALSE
)
